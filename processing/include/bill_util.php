...
class RegistrarClient {
	...
	public function __construct($url, $login, $password, $debug = false) {
		
		$this->client = new Json_RPC_Client($url);
		$this->client->authentication($login, $password);
		
		if ($debug){
			$this->client->debug = true;
			$this->client->ssl_verify_peer = false;
		}
	}
...
// 	
	...
	public function CreateDomain($domain, $period, $ns, $registrant_info) {
		try{
			$response = $this->client->CreateDomain( array(
															'domain' => $domain,
															'period' => $period,
	...
	public function Prolong($domain, $period, $ex_year){
		try{
		$response = $this->client->RenewDomain( array(
														'domain' => $domain,
														'period' => $period,
														'exDateYear' => $ex_year,
													  ) );
	...
	public function UpdateDomain($domain, $registrant_info) {
		try{
	...
	public function UpdateNS($domain, $ns) {
		try{
		$response = $this->client->UpdateDomain( array(
														'domain' => $domain,
														's_ns' => $ns,
